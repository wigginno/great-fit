(function() {
  ...
  return (async () => {
    ...
    try {
      ...
      turndownService.remove(['script', 'style', 'noscript', 'iframe', 'link', 'meta']);
      console.log('Added rules to remove script, style, noscript, iframe, link, and meta tags.');
      // --------------------------------------------
      ...
      turndownService.addRule('removeJsonBlobs', {
        filter: function (node) {
          ...
          if (node.nodeName === 'PRE' && node.textContent.length > 1000 && 
              (node.textContent.trim().startsWith('{') || node.textContent.trim().startsWith('['))) {
            return true;
          }
          
          // 2. Check for code blocks with JSON
          if (node.nodeName === 'CODE' && node.textContent.length > 1000 && 
              (node.textContent.trim().startsWith('{') || node.textContent.trim().startsWith('['))) {
            return true;
          }
          ...
          if (node.attributes && node.attributes.length > 0) {
            for (let i = 0; i < node.attributes.length; i++) {
              ...
              if (attr.name.startsWith('data-') && attr.value.length > 500 && 
      ...
      console.log('Converted Markdown (content script log):', markdownContent.substring(0, 500) + '...'); // Log start of Markdown
