...
async function handleClipboardWrite(data) {
  ...
  try {
    ...
    console.log('Text successfully copied to clipboard by offscreen document (using execCommand).');
  } catch (error) {
    console.error(`Offscreen document failed to copy text using execCommand. Error Name: ${error.name}, Message: ${error.message}`);
  } finally {
